...
class FeatureTable extends React.Component {
	...
	_onLayerSelectChange( layer ) {
		// TODO add clearing filter back ReactDOM.findDOMNode(this.refs.filter).value =
		// '';
	...
	_onChangeLayers( ) {
		...
		for ( var i = 0, ii = flatLayers.length; i < ii; ++i ) {
			...
			if (lyr.getVisible( ) && lyr.get( 'title' ) !== null && ( lyr instanceof ol.layer.Vector || lyr.get( 'wfsInfo' ) !== undefined )) {
				//FORCE SELECTED LAYER to USER Layer
				if ( !this._layer && lyr.get( 'name' ) === appConfig.layer ) {
					this._layer = lyr;
				}
				if ( lyr.get( 'name' ) === appConfig.layer ) {
					layers.push( lyr );
				}
	...
	render( ) {
		...
		var columns = [
			{
				...
				render: function ( props ) {
					...
					return ( <Checkbox
						...
						checked={selected}
						onCheck={me._onSelect.bind( me, props )}/> );
				}
			}
		];
		...
		if ( checkToCreateTable ) {
			...
			table = ( <ReactTable
				...
				pages={this._layer instanceof ol.layer.Vector
				? undefined
				: this.state.pages}
				data={data}
				manual={!( this._layer instanceof ol.layer.Vector )}
				showPageSizeOptions={false}
				onChange={( this._layer instanceof ol.layer.Vector )
				? undefined
				: this._onTableChange.bind( this )}
				showPageJump={false}
		...
		return (
			<Paper
				zDepth={0}
				className={classNames( 'sdk-component featureTable', this.props.className )}
				style={style}>
				<Toolbar ref='form' className="featureTableToolbar">
					<ToolbarGroup firstChild={true}>
						<LayerSelector
							{...this.props}
							id='table-layerSelector'
							disabled={!this._layer}
							ref='layerSelector'
							onChange={this._onLayerSelectChange.bind( this )}
					...
					<ToolbarGroup>
						<TextField
							...
							floatingLabelText={formatMessage( messages.filterlabel )}
							id='featuretable-filter'
							disabled={!this._layer}
							ref='filter'
							onChange={this._filterByText.bind( this )}
							hintText={formatMessage( messages.filterplaceholder )}/>
						<FilterHelp intl={this.props.intl}/>
						<Button
							buttonType='Icon'
							disabled={!this._layer}
							iconClassName='ms ms-crosshair'
							tooltip={formatMessage( messages.zoombuttontitle )}
							onTouchTap={this._zoomSelected.bind( this )}/>
						<IconMenu
							anchorOrigin={{
							horizontal: 'right',
							vertical: 'top'
							...
							<MenuItem
								primaryText={formatMessage( messages.clearbuttontitle )}
								disabled={!this._layer}
								onTouchTap={this._clearSelected.bind( this )}/>
							<MenuItem
								primaryText={< Toggle label = {
